...
class UpdateFinancialReportRequest extends FormRequest
{
    ...
    {
        $financialReportId = $this->route('financial_report')->id ?? null;

        return [
            'fiscal_year' => ['required', 'integer', 'min:1900', 'max:'.(date('Y') + 10), 'unique:financial_reports,fiscal_year,'.$financialReportId],
